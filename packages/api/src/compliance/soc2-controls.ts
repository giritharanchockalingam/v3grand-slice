...
// G-14/F-11: Formal control matrices and testing procedures.
//
// This module defines the control framework for V3 Grand Investment OS
// aligned with SOC 2 Type II (Trust Services Criteria) and ISAE 3402.
//
// Control categories map to TSC criteria:
...
export const SOC2_CONTROLS: Control[] = [
  // ── CC5: Control Activities — Engine Integrity ──
  {
    id: 'CC5-01',
    tscCriteria: 'CC5.1',
...
/**
 * Get all controls for a specific TSC criteria.
 */
export function getControlsByCriteria(criteria: string): Control[] {
  return SOC2_CONTROLS.filter(c => c.tscCriteria === criteria);
}

/**
 * Get controls by type (preventive, detective, corrective).
 */
export function getControlsByType(type: Control['controlType']): Control[] {
  return SOC2_CONTROLS.filter(c => c.controlType === type);
}

/**
 * Generate a control matrix summary for audit reporting.
 */
export function generateControlMatrix(): {
